      ...
      <div className='modern-card overflow-hidden p-0 h-full flex flex-col'>
        <Link onClick={() => { try { scrollTo(0, 0) } catch (e) { } }} to={`/product/${id}`} className='block'>
          ...
          <div className='p-4 flex-1 flex flex-col'>
            ...
            <div className='mt-auto'>
              <p className='text-xl font-bold text-orange-600'>{currency}{formatPrice(price)}</p>
            ...
            <div className='flex gap-2 mb-4 flex-wrap'>
              {sizes.map((size) => (
                <button
                  ...
                  className={`px-3 py-2 text-sm rounded-lg font-medium transition-all ${selectedSize === size
        ...
        <div className='absolute bottom-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300'>
          <button
            ...
            className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-200 hover:scale-110 shadow-lg ${wishlist.find(item => item && item._id && item._id.toString() === id?.toString()) ? 'bg-red-500 text-white' : 'bg-white text-orange-600 hover:text-red-500'}`}
          >
            <LucideHeart size={20} fill={wishlist.find(item => item && item._id && item._id.toString() === id?.toString()) ? 'currentColor' : 'none'} />
          </button>
          <button
            onClick={handleAddToCart}
            className='w-10 h-10 bg-orange-500 text-white rounded-full flex items-center justify-center hover:bg-orange-600 transition-all duration-200 hover:scale-110 shadow-lg'
          >
